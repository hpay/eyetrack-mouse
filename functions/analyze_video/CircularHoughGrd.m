function [accum, varargout] = CircularHoughGrd(img, radrange, varargin)
...
prm_LM_LoBndRa = 0.4;  % minimum ratio of LM to the max of 'accum' %0.2 HP***
...
img_is_double_or_single = isa(img, 'double') || isa(img, 'single');
if ~img_is_double_or_single
    img = single(img);
end
...
if prm_useaoi,
    ...
    accumAOI = ones(0,4);
    for k = 1 : accum_nRgn,
        [accumrgn_IdxI, accumrgn_IdxJ] = find( accumlabel == k ); % Edit HP        
...
if dbg_on,
    ...
    line(circen(:,1),circen(:,2),'LineStyle','none','Marker','+','Color','m','LineWidth',2,'MarkerSize',10); %HP 6/26/14
